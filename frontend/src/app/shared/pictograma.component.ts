...
/**
 * Nombres del repertorio de pictogramas.
 *
 * El conjunto es cerrado a proposito: un pictograma nuevo se dibuja aqui, con
 * la misma retícula y el mismo peso que los demas, en lugar de importarse de
 * una libreria distinta.
 */
...
/**
 * Trazados macizos, dibujados sobre una retícula de 24 unidades.
 *
 * Todos comparten la gramática de la señalética de transporte: silueta llena,
 * sin contorno, geometría legible a tamaño pequeño y a distancia. Es la razón
 * de dibujarlos en vez de importar una librería de iconos de trazo fino: un
 * icono de línea de 1,5 px pertenece al mundo del panel de administración
 * genérico, no al de un cartel de sala de control.
 */
/**
 * Un pictograma es uno o varios trazados.
 *
 * La mayoria se resuelve con un solo trazado y `evenodd`, que perfora los
 * huecos interiores. Cuando una forma se superpone a otra y debe *sumarse* en
 * lugar de perforarla —la barra diagonal del ojo tachado, la punta de la
 * flecha de restauracion—, se declaran trazados separados: cada uno se rellena
 * por su cuenta y la union es aditiva.
 */
const TRAZADOS: Record<NombrePictograma, string | readonly string[]> = {
  // Arco de acceso: la puerta por la que se pasa al control.
  sala: 'M2 21v-8.5C2 7.25 6.48 3 12 3s10 4.25 10 9.5V21h-4.6v-8.5c0-2.9-2.42-5.2-5.4-5.2s-5.4 2.3-5.4 5.2V21H2z',
  // Sello sobre documento: el trámite marcado en ventanilla.
  ...
  // Globo con meridianos: el trámite que cruza fronteras.
  ...
  // Libreta de pasaporte con su emblema.
  ...
  // Hoja de arce: el trámite canadiense.
  ...
  // Libro de registro foliado: la bitácora de ventanilla.
  ...
  // Llave: la credencial custodiada. Solo aparece en dorado.
...
/**
 * Pictograma del sistema.
 *
 * Decorativo por omisión (`aria-hidden`): el rótulo que acompaña al
 * pictograma es lo que leen los lectores de pantalla. Cuando el pictograma
 * viaja solo, quien lo usa pasa `etiqueta` y pasa a anunciarse como imagen con
 * nombre accesible.
 */
@Component({
  selector: 'app-picto',
...
export class PictogramaComponent {
  ...
  /** Normaliza a lista: un pictograma de un solo trazado también se recorre. */
