...
from app.schemas import MasterTramexBase, MasterTramexResponse, MasterTramexUpdate

router = APIRouter()


@router.get("/", response_model=list[MasterTramexResponse])
def listar(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Listar registros de master_tramex con paginación."""
    return db.query(MasterTramex).offset(skip).limit(limit).all()
...
def crear(datos: MasterTramexBase, db: Session = Depends(get_db)):
...
def actualizar(registro_id: int, datos: MasterTramexUpdate, db: Session = Depends(get_db)):
